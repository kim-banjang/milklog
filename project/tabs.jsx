/* MilkLog — tab panes: HomeTab (input + alarm), LogTab (list/calendar), MyTab. */
      ...
      {alarm.active ? (
        <div className={`ml-countdown${overdue ? ' is-overdue' : ''}`}>
          ...
          <div className="ml-cd-foot">
            <span className="ml-cd-at">예정 {mlFmtTime(alarm.nextAt)}</span>
        ...
        <button type="button" className="ml-alarm-set" onClick={onToggleAlarm}>
          {mlIntervalLabel(alarm.intervalMin)} 간격으로 알람 켜기
        </button>
      )}
    </div>
  );
...
function HomeTab({ prepared, leftover, setPrepared, setLeftover, onRecord, alarm, now, cd, onSetInterval, onToggleAlarm }) {
  return (
  ...
  return (
    <div className="ml-tabpane">
      <div className="ml-segment">
        <button type="button" className={`ml-seg-btn${logView === 'list' ? ' is-active' : ''}`}
          onClick={() => setLogView('list')}>
          <NoteIcon size={18} /> 리스트
        </button>
        <button type="button" className={`ml-seg-btn${logView === 'calendar' ? ' is-active' : ''}`}
            ...
            return (
              <div key={k} className="ml-card ml-day-card">
                <button type="button" className="ml-day-head ml-day-toggle"
                  ...
                  <span className="ml-day-title-sm">{mlFmtDateLong(k)}</span>
                  <span className="ml-day-meta">
...
function MyTab({ profile, now, todayCount, todayTotal, totalRecords, dailyAvg }) {
  return (
    <div className="ml-tabpane">
      <div className="ml-card ml-profile">
        ...
        <div className="ml-profile-info">
          ...
          <div className="ml-profile-meta">{profile.birth} · {mlAgeString(profile.birth, now)}</div>
        </div>
      </div>
      ...
      <div className="ml-card ml-avg-card">
        ...
        <div className="ml-avg-value">
          <span className="ml-num">{dailyAvg}</span><span className="ml-avg-unit">ml</span>
        </div>
      </div>
    </div>
  );
...
Object.assign(window, { HomeTab, LogTab, MyTab, AlarmCard, StatCell });
