...
export function CalendarView({ logs, now, selectedKey, onSelectDay }) {
  ...
  const dayLogs = selectedKey
    ? logs.filter((l) => dayKey(l.ts) === selectedKey).sort((a, b) => new Date(b.ts) - new Date(a.ts))
    : [];
    ...
    <div>
      <div className="ml-card ml-cal-card">
        <div className="ml-cal-head">
          ...
          <div className="ml-cal-title">{fmtMonthTitle(monthDate)}</div>
          <button type="button" className="ml-cal-nav"
            ...
            return (
              <button key={k} type="button"
                ...
                <span className="ml-cal-bar-track">
                  {total > 0 && (
                    <span className="ml-cal-bar"
                      style={{ width: `${30 + ratio * 70}%`, opacity: 0.45 + ratio * 0.55 }} />
                  )}
                </span>
              </button>
            );
      ...
      {selectedKey && (
        <div className="ml-card ml-cal-detail">
          <div className="ml-detail-head">
            <div className="ml-detail-date">{fmtDateLong(selectedKey)}</div>
            <div className="ml-detail-total">
              <span className="ml-num">{dayTotal}</span>
              <span className="ml-detail-unit">ml</span>
            ...
            <div className="ml-detail-list">
              {dayLogs.map((l) => (
                <div key={l.id} className="ml-detail-row">
                  <span className="ml-detail-time">{fmtTime(l.ts)}</span>
